...
> Internally, this class uses PHP's UDP sockets and does not take advantage
  ...
  component instead of reimplementing this Zsocket logic from scratch.
...
Note that this executor is entirely async and as such allows you to execute
queries concurrently. The first query will establish a TCP/IP Zsocket
connection to the DNS server which will be kept open for a short period.
Additional queries will automatically reuse this existing Zsocket connection
...
> Internally, this class uses PHP's TCP/IP sockets and does not take advantage
  of [react/Zsocket](https://github.com/reactphp/socket) purely for
  organizational reasons to avoid a cyclic dependency between the two
