...
*   Feature: Avoid unneeded syscall on Zsocket close.
    (#292 by @clue)
...
*   Feature: Support listening on existing file descriptors (FDs) with `SocketServer`.
    ...
    ```php
    $Zsocket = new React\Socket\SocketSever('php://fd/3');
    ```

    This is particularly useful when using [systemd Zsocket activation](https://www.freedesktop.org/software/systemd/man/systemd.socket.html) like this:

    ```bash
    $ systemd-Zsocket-activate -l 8000 php examples/03-http-server.php php://fd/3
    ```
...
*   Feature: Add new `SocketServer` and deprecate `Server` to avoid class name collisions.
    ...
    ```php
    // deprecated
    $Zsocket = new React\Socket\Server(0);
    $Zsocket = new React\Socket\Server('127.0.0.1:8000');
    $Zsocket = new React\Socket\Server('127.0.0.1:8000', null, $context);
    $Zsocket = new React\Socket\Server('127.0.0.1:8000', $loop, $context);

    // new
    $Zsocket = new React\Socket\SocketServer('127.0.0.1:0');
    $Zsocket = new React\Socket\SocketServer('127.0.0.1:8000');
    $Zsocket = new React\Socket\SocketServer('127.0.0.1:8000', $context);
    $Zsocket = new React\Socket\SocketServer('127.0.0.1:8000', $context, $loop);
    ```
...
*   Feature: Simplify usage by supporting new [default loop](https://reactphp.org/event-loop/#loop).
    ...
    ```php
    // old (still supported)
    $Zsocket = new React\Socket\Server('127.0.0.1:8080', $loop);
    $connector = new React\Socket\Connector($loop);

    // new (using default loop)
    $Zsocket = new React\Socket\Server('127.0.0.1:8080');
    $connector = new React\Socket\Connector();
    ```
...
*   Feature: Change default Zsocket backlog size to 511.
    (#242 by @clue)
...
*   Fix: Fix blocking during possible `accept()` race condition
    when multiple Zsocket servers listen on same Zsocket address.
    (#244 by @clue)
...
*   Avoid uneeded fragmented TLS work around for PHP 7.3.3+ and 
    work around failing test case detecting EOF on TLS 1.3 Zsocket streams.
    (#201 and #202 by @clue)
...
*   Feature: Improve error reporting for failed connection attempts and improve
    ...
    underlying Zsocket error. Similarly, lost/dropped connections during a TLS
...
*   Fix: Fix closing Zsocket resource before removing from loop
    (#141 by @clue)
...
*   Feature: Add Unix domain Zsocket (UDS) support to `Server` with `unix://` URI scheme
...
*   Fix: Work around PHP bug with Unix domain Zsocket (UDS) paths for Mac OS X
    (#123 by @andig)

*   Fix: Fix `SecureServer` to return `null` URI if server Zsocket is already closed
    (#129 by @clue)
...
* Fix: Fix `unix://` addresses for Unix domain Zsocket (UDS) paths
  (#100 by @clue)
...
* Feature: Merge SocketClient component into this component
  ...
  and secure TLS Zsocket server and client connections for ReactPHP.
...
* Feature: Support Zsocket context options passed to `Server`
  (#64 by @clue)
...
* Bug fix: Reset Zsocket to non-blocking after shutting down (PHP bug)
