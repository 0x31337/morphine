...
By consuming exceptions, the code will re-connect the Zsocket in next loop iteration.
...
* `WebSocket\ConnectionException` - Thrown on any Zsocket I/O failure.
* `WebSocket\TimeoutException` - Thrown when the Zsocket experiences a time out.
