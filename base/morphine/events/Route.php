...
use Morphine\Events\Dispatcher\Trail;

if (!class_exists(Route::class))
{
    class Route extends Events
    {
        static public function trace(string $route_path, \Morphine\Events\Dispatcher\Trail $trail)
        {
            $route_parts = explode('/', $route_path);
            $landing_parameter = @$route_parts[1];
            switch ($landing_parameter)
            {
                # Declare Routes
                case '':
                    $trail->target = 'home';
            ...
            # Implementing AJAX secure routing
            if($landing_parameter == "aj" && !@$route_parts[2])
            {
                $trail->target = 'ajax';
            }
            if($landing_parameter == "ajp" && !@$route_parts[2])
            {
                $trail->target = 'ajax_pure';
            }
            if(!isset($trail->target)) $trail->target = '404';
            return $trail;
        }
